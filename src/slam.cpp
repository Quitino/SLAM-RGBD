...
FRAME readFrame(int index, string vstrImageFilenamesRGB, string vstrImageFilenamesD, ParameterReader& pd );
...
int main( int argc, char** argv )
{
    ...
    FRAME currFrame = readFrame(0, vstrImageFilenamesRGB[0], vstrImageFilenamesD[0], pd );
    ...
    *******************************/
    ...
    for (int i=1; i<vTimestamps.size()-2500; i++)
    {
        FRAME currFrame = readFrame(i, vstrImageFilenamesRGB[i],vstrImageFilenamesD[i],pd ); // 读取currFrame
    ...
    // //体素滤波器
    // pcl::VoxelGrid<PointT> voxel; // 网格滤波器，调整地图分辨率//pointT在slambase.h中定义了
    ...
    for (size_t i=0; i<keyframes.size(); i++)
    {
        ...
        // 滤波部分
        // if(newCloud->size() == 0)
        //     cout<<"newCloud is zero!"<<endl;
        // pcl::VoxelGrid<PointT> voxel1; // 网格滤波器，调整地图分辨率//pointT在slambase.h中定义了
        // voxel1.setInputCloud( newCloud );
        // double gridsize = atof( pd.getData( "voxel_grid" ).c_str() ); //分辨图可以在parameters.txt里调
        // voxel1.setLeafSize( gridsize, gridsize, gridsize );
        // voxel1.filter( *tmp );
    ...
    pass.setFilterFieldName("z");
    pass.setFilterLimits( 0.0, 1.0 ); //4m以上就不要了
...
CHECK_RESULT checkKeyframes( FRAME& f1, FRAME& f2, g2o::SparseOptimizer& opti, bool is_loops)
{
    ...
    // Eigen::Isometry3d T1 = cvMat2Eigen( result.rvec, result.tvec );
    // cout<<"T1="<<T1.matrix()<<endl;
    cout<<"solve pnp done!"<<endl;
